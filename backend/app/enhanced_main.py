...
@app.get("/predictions/{currency}/best")
async def get_best_predictions(
    ...
    try:
        # Get best predictions from database
        predictions = await db_manager.get_best_predictions(currency, days, limit)
        ...
        logger.error(f"Error fetching best predictions for {currency}: {e}")
        raise HTTPException(status_code=500, detail="Error fetching best predictions")

@app.get("/predictions/accuracy/{currency}")
async def get_prediction_accuracy(currency: str, days: int = 30):
    """Get prediction accuracy metrics for a currency using best predictions"""
    try:
        # Get best predictions with validation data
        predictions = await db_manager.get_best_predictions(currency, days, 1000)
