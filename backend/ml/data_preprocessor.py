...
class CryptoDataPreprocessor:
    ...
    def create_labels(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Using a higher threshold to reduce noise and create more balanced classes
        threshold = 1.5  # 1.5% threshold (increased from 0.5%)
        df['target'] = (df['price_change_pct'] > threshold).astype(int)
        
        # Log the target distribution for debugging
        target_dist = df['target'].value_counts()
        logger.info(f"Target distribution after {threshold}% threshold: {target_dist.to_dict()}")
