...
async def generate_prediction_for_currency(currency: str) -> Dict[str, Any]:
    """
    Generate predictions for all models for a specific currency
    ...
    try:
        ...
        # Make and save predictions for all models
        predictions = await pipeline.make_and_save_all_predictions(currency)
        ...
        for prediction in predictions:
            ...
            logger.info(f"✅ {currency} prediction ({model_type}): {prediction['predicted_direction']} "
                       f"(confidence: {prediction['confidence_score']:.2%})")
        
        logger.info(f"✅ {currency} predictions complete! Generated {len(predictions)} predictions")
        ...
        logger.error(f"❌ Error generating predictions for {currency}: {str(e)}")
        return {
            'currency': currency,
            'success': False,
            'error': str(e)
...
async def generate_historic_predictions(currency: str, days: int = 90) -> Dict[str, Any]:
    ...
    while current_date <= end_date:
        try:
            ...
            # Make predictions for all models for this date
            predictions = await pipeline.make_predictions_for_all_models(currency, prediction_datetime)
            ...
            logger.warning(f"   Failed to generate predictions for {current_date}: {str(e)}")
            errors += 1
...
async def generate_current_predictions():
    ...
    for currency, result in results.items():
        if result['success']:
            ...
            print(f"✅ {currency}: Generated {len(predictions)} predictions")
            for prediction in predictions:
                model_type = prediction['model_metadata']['model_type']
                print(f"   - {model_type}: {prediction['predicted_direction']} "
