...
// Helper function to extract model type from model version
const extractModelType = (modelVersion: string): string => {
  if (!modelVersion || modelVersion === 'unknown') return 'unknown'
  
  // Handle model versions like 'random_forest_20250725_200145' or 'logistic_regression_20250725_200416'
  const parts = modelVersion.split('_')
  if (parts.length >= 2) {
    // Check if it's a two-word model type (random_forest, logistic_regression)
    if (parts[0] === 'random' && parts[1] === 'forest') {
      return 'random_forest'
    } else if (parts[0] === 'logistic' && parts[1] === 'regression') {
...
export default function PredictionsPage() {
  ...
  const fetchPredictionData = useCallback(async () => {
    try {
      ...
      const transformedPredictions: PredictionHistoryItem[] = predictionHistory.combined.map(pred => ({
        ...
        model_type: extractModelType(pred.model_version || 'unknown')
      }))
  ...
  const calculateRealAccuracy = useCallback((predictions: PredictionHistoryItem[]): AccuracyMetrics => {
  ...
  const calculateRealModelPerformance = useCallback((predictions: PredictionHistoryItem[]): ModelPerformance[] => {
    if (!predictions || predictions.length === 0) return []
    
    const modelTypes = ['logistic_regression', 'random_forest', 'lstm', 'unknown']
    const performance: ModelPerformance[] = []
    
    modelTypes.forEach(modelType => {
      const modelPredictions = predictions.filter(p => {
        const predictionModelType = extractModelType(p.model_version || '')
        return predictionModelType === modelType
      })
      ...
      const correctPredictions = validatedPredictions.filter(p => p.is_correct === true)
      const accuracy = validatedPredictions.length > 0 ? (correctPredictions.length / validatedPredictions.length) * 100 : 0
      ...
      performance.push({
        ...
        avg_confidence: Math.round(avgConfidence * 100 * 10) / 10, // Convert to percentage
  ...
  const getDirectionDistribution = () => {
    ...
    // Filter out Random Forest predictions for the distribution chart
    const nonRFPredictions = filtered.filter(p => 
      !p.model_version?.toLowerCase().includes('random_forest')
    )
    
    const upPredictions = nonRFPredictions.filter(p => p.predicted_direction === 'UP').length
    const downPredictions = nonRFPredictions.filter(p => p.predicted_direction === 'DOWN').length
